...
const Home = () => {
    ...
    const handleRegionSelect = async (region) => {
      ...
      try {
        ...
        if (response.ok) {
          navigate('/options'); // Use navigate instead of history.push
        } else {
                    ...
                    {dropdownOpen && (
                        <div className="bg-[#EDEDED] divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700">
                            <ul className="py-2 text-sm text-gray-700 dark:text-gray-200">
                                <li>
                                    <button onClick={() => handleRegionSelect('Nairobi')}
                                            className="block px-4 py-2 w-full text-left hover:bg-white">Nairobi</button>
                                </li>
                                <li>
                                    <button onClick={() => handleRegionSelect('Coastal')}
                                            className="block px-4 py-2 w-full text-left hover:bg-white">Coastal</button>
                                </li>
                                <li>
                                    <button onClick={() => handleRegionSelect('Western')}
                                            className="block px-4 py-2 w-full text-left hover:bg-white">Western</button>
                                </li>
                            </ul>
                        </div>
                    )}
